...
def main():
    print("V6 Worker Supervisor started.", flush=True)
    while True:
        started_at = time.time()
        proc = subprocess.Popen([sys.executable, "live_worker.py"])
        ...
        while proc.poll() is None:
            ...
            age = _heartbeat_age()
            # Give a newly started worker time to write its first heartbeat.
